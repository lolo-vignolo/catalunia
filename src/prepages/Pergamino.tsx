...
// Utilidad para manejo seguro de localStorage
const localStorageUtils = {
  ...
  getQrList: (): string[] => {
    try {
      ...
      const qrList = localStorage.getItem("qr-list");
      return qrList ? qrList.split(",").filter(id => id.trim()) : [];
    } catch (error) {
      console.warn("Error leyendo qr-list:", error);
      return [];
    }
  },

  saveQrList: (qrList: string[]): boolean => {
    try {
      if (!localStorageUtils.isAvailable()) return false;
      const listString = qrList.join(",");
      localStorage.setItem("qr-list", listString);
      
      // Verificar que se guardó correctamente
      const saved = localStorage.getItem("qr-list");
      return saved === listString;
    } catch (error) {
      console.warn("Error guardando qr-list:", error);
      return false;
    }
  },

  addQrId: (newId: string): boolean => {
    const currentList = localStorageUtils.getQrList();
    ...
    return localStorageUtils.saveQrList(updatedList);
  }
};
...
const Pergamino = () => {
  ...
  useEffect(() => {
    if (!isLocalStorageReady || !id) return;

    // Usar requestAnimationFrame para mejor timing en Safari
    requestAnimationFrame(() => {
      const success = localStorageUtils.addQrId(id);
      
      if (!success) {
        // Retry después de un delay más largo para Safari
        setTimeout(() => {
          localStorageUtils.addQrId(id);
        }, 500);
      }
      
      // Debug logging (remover en producción)
      if (process.env.NODE_ENV === 'development') {
        console.log('QR ID agregado:', id);
        console.log('Lista actual:', localStorageUtils.getQrList());
      }
    });
  }, [id, isLocalStorageReady]);
